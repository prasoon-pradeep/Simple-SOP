...
### PHASE 13 — Auto-Update

**Prerequisite:** Make the GitHub repo public. GitHub Release assets on private repos require auth tokens which breaks Tauri's updater — public repos serve assets as plain URLs that the updater can fetch directly.

**Implementation steps:**
1. Add `tauri-plugin-updater` to `Cargo.toml` and register in `lib.rs`
2. Generate signing keypair once: `npm run tauri signer generate` — store private key as GitHub Secret `TAURI_SIGNING_PRIVATE_KEY`, embed public key in `tauri.conf.json` under `plugins.updater.pubkey`
3. Configure update endpoint in `tauri.conf.json`: `"endpoints": ["https://github.com/prasoon-pradeep/Simple-SOP/releases/latest/download/latest.json"]`
4. Modify `.github/workflows/build-linux.yml` and `build-windows.yml` to: sign artifacts with private key, generate `latest.json` with version + per-platform download URLs + signatures, create a GitHub Release and upload all artifacts + `latest.json`
5. Add update check on app launch in Rust (async, non-blocking) — if update available, emit event to frontend
6. Frontend shows a small "Update available — install now?" banner or dialog; user confirms → updater downloads, installs, and restarts app

**Windows note:** Without a paid code signing certificate, Windows SmartScreen will show an "unrecognised app" warning on the initial `.exe`/`.msi` install. Once installed, subsequent updates via the updater are silent and unaffected.

**`latest.json` format (generated by CI):**
```json
{
  "version": "0.2.0",
  ...
  "platforms": {
    "linux-x86_64":   { "url": "https://.../.AppImage", "signature": "..." },
    "windows-x86_64": { "url": "https://.../.msi",      "signature": "..." }
  }
}
```
