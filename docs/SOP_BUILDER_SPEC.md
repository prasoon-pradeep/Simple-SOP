...
**Stable URL (hardcoded once in `tauri.conf.json`, never changes):**
```
https://github.com/prasoon-pradeep/Simple-SOP/releases/latest/download/latest.json
```
The word `latest` is a first-class GitHub feature — it always redirects to the most recent release's assets automatically.

**How the app uses it:** On launch, the updater fetches this URL, reads the `version` field, compares to the running version. If newer → prompts user. The JSON also contains the signed download URL for each platform so the app knows exactly what to fetch.

**Format (generated by CI):**
```json
{
  "version": "0.2.0",
